...
const AddToCartButton: React.FC<AddToCartButtonProps> = ({ currentProduct }) => {
  ...
  const handleAddToCart = () => {
    try {
        ...
        if (user?.id) {
          ...
          const old_cart_data = localStorage.getItem("cart");
          if (old_cart_data) {
            var old_cart = eval(JSON.parse(old_cart_data))
            if (old_cart.hasOwnProperty(currentProduct.id)) {
              dispatch(
                successToast({
                toast: true,
                message: "Item already Added to Cart..!!",
  ...
  return (
    ...
    <button
      className="group p-2 border border-gray-400 rounded-md bg-gray-100 text-gray-600 font-semibold text-lg flex items-center justify-center gap-2 shadow-sm shadow-transparent transition-all duration-500 hover:bg-gray-200 hover:shadow-indigo-200"
