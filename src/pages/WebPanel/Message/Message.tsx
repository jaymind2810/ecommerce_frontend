...
import { MessagesDataType } from "../Type/MessageType";
...
function Message() {
  ...
  // const { send } = useWebSocket("ws://localhost:8000/ws/chat/", handleMessage);
  const { socketDataSend } = useWebSocket(SOCKET_URL, handleMessage);
  ...
  function handleMessage(type: string, data: any) {
    if (type === "chat") {
      console.log("Private chat message:", data);
  ...
  //     // setMessages((prev) => [...prev, data.response || data.message]);
  //   };
